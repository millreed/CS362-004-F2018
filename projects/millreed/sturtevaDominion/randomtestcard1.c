...
int main() {
	...
	for (n = 0; n < tests; n++) {
		...
		if (smithy != 0){
			printf("Test#%d: Failed, Smithy function failed for input: handCount: %d, deckCount: %d, discardCount: %d\n", n, GPrime.handCount[p], GPrime.deckCount[p], GPrime.discardCount[p]);
			failures++;
		}
			...
			if(GPrime.handCount[p] + 2 != G.handCount[p]){
				printf("Test#%d Expected %d handcount, found %d handcount for input: handCount: %d, deckCount: %d, discardCount: %d\n", n, GPrime.handCount[p] + 2, G.handCount[p], GPrime.handCount[p], GPrime.deckCount[p], GPrime.discardCount[p]);
				failures++;
			}
			// check the handcount after playing the card
			else if(GPrime.deckCount[p] > 3){
				if (GPrime.deckCount[p] - 3 != G.deckCount[p]){
					printf("Test#%d: Expected %d deckCount, found %d deckCount for input: handCount: %d, deckCount: %d, discardCount: %d\n", n, GPrime.deckCount[p] - 3 , G.deckCount[p], GPrime.handCount[p], GPrime.deckCount[p], GPrime.discardCount[p]);
					failures++;
				}
			}
			else {
				if (GPrime.deckCount[p] + GPrime.discardCount[p] - 3 != G.deckCount[p]){
					printf("Test#%d: Expected %d deckCount, found %d deckCount for input: handCount: %d, deckCount: %d, discardCount: %d\n", n, GPrime.deckCount[p] + GPrime.discardCount[p] - 3 , G.deckCount[p], GPrime.handCount[p], GPrime.deckCount[p], GPrime.discardCount[p]);
					failures++;
				}
			}
		}
