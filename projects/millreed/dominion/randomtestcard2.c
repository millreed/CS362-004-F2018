...
int main() {
	...
	for (n = 0; n < tests; n++) {
		...
		if (council_room != 0){
			printf("Test#%d: Failed, Council Room function failed", n);
			failures++;
		}
			...
			if (GPrime.numBuys+1 != G.numBuys){
				printf("Test#%d: Expected %d buys, found %d buys\n", n, GPrime.numBuys+1 , G.numBuys);
				pass &= 0;
			}
			...
			for (i = 0; i < numPlayer; i++){
				if (i != p){
					if (GPrime.handCount[i] + 1 != G.handCount[i]){
						printf("Test#%d: Expected %d handcount for Player %d, found %d handcount\n", n, GPrime.handCount[i]+1, i, G.handCount[i]);
						pass &= 0;
					}
				}
			}
