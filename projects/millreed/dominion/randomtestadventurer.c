...
int main() {
	...
	for (n = 0; n < tests; n++) {
		...
		if (adventurer != 0){
			printf("Test#%d: Failed, Adventurer function failed", n);
			failures++;
		}
			...
			// check the handcount after playing the card
				...
				printf("Test#%d: Expected %d handcount, found %d handcount\n", n, GPrime.handCount[p] + 1, G.handCount[p]);
				failures++;
			}
		}
